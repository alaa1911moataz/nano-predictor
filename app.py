...
<style>
    ...
    div[data-baseweb="select"] > div:focus-within, .stNumberInput input:focus, .stTextInput input:focus {{ border: 1px solid #7C4DFF !important; box-shadow: 0 0 0 3px rgba(124, 77, 255, 0.15) !important; }}
    label {{ color: {label_color} !important; font-weight: 500 !important; font-size: 0.86rem !important; }}

    div.stButton > button {{ width: 100%; background: linear-gradient(100deg, #7C4DFF, #00BFA5); color: white; border: none; border-radius: 13px; padding: 0.85rem 1rem; font-size: 1.02rem; font-weight: 700; letter-spacing: 0.3px; display: flex; align-items: center; justify-content: center; transition: all 0.25s ease; box-shadow: 0 6px 20px rgba(124, 77, 255, 0.25); }}
...
<div class="hero-wrap">
    ...
    <div class="hero-subtitle">
        Predict if nano-particles will achieve <b style="color:#7C4DFF;">High Tumor Retention</b> 
        and optimal <b style="color:#00A896;">Selectivity</b> using an advanced XGBoost Classification model.
    </div>
</div>
""", unsafe_allow_html=True)
    ...
    if predict_clicked:
        ...
        with res_col1:
            ...
            <div class="metric-card-custom">
                <div class="metric-label-custom">📊 Tumor Retention Potential</div>
                <div class="{tumor_class}">{tumor_res}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with res_col2:
            st.markdown(f"""
            <div class="metric-card-custom">
                <div class="metric-label-custom">🎯 Targeting Selectivity Index</div>
                <div class="{selectivity_class}">{selectivity_res}</div>
            </div>
            """, unsafe_allow_html=True)
