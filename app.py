...
# ---------------- Load model + real training categories ----------------
@st.cache_resource
def load_nano_resources():
    pipeline = joblib.load("xgboost_nano_classifier_model.pkl")
    categories = joblib.load("nano_categories.pkl")
...
def options_for(col, categories):
    """Returns the real values the model was trained on for this column (excluding 'nan')"""
    vals = [v for v in categories.get(col, []) if v != "nan"]
    return [""] + vals

try:
    model, categories = load_nano_resources()
    ...
    with col2:
        shape = st.selectbox("Shape", options_for("Shape", categories))
        size_cat = st.selectbox("Size Category", options_for("Size_Category", categories))
        zeta_cat = st.selectbox("Zeta Category", options_for("Zeta_Category", categories))
    ...
    with col3:
        np_class = st.selectbox("NP Class", options_for("NP_Class", categories))
        inps_core = st.selectbox("INPs Core", options_for("INPs_Core", categories))
        has_peg = st.selectbox("Has PEG", options_for("HAS_PEG", categories))
    with col4:
        shell_type = st.selectbox("Shell Type", options_for("Shell Type", categories))
        organ = st.selectbox("Organ or Tissue", options_for("Organ or tissue", categories))
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col6:
        tumor_site = st.selectbox("Tumor Site", options_for("Tumor Site", categories))
    st.markdown('</div>', unsafe_allow_html=True)

    predict_clicked = st.button("🔬 Classification Screening", type="primary", use_container_width=True)

    if predict_clicked:
        # Build the row with the exact same shape as the training data
        raw_row = {
            "NP_Class": np_class if np_class else "nan",
            "INPs_Core": inps_core if inps_core else "nan",
            "Shape": shape if shape else "nan",
            "Size (nm)": float(size),
            "Size_Category": size_cat if size_cat else "nan",
            "Zeta Potential (mv)": float(zeta_mv) if zeta_mv.strip() != "" else np.nan,
            "Zeta_Category": zeta_cat if zeta_cat else "nan",
            "Organ or tissue": organ if organ else "nan",
            "HAS_PEG": has_peg if has_peg else "nan",
            "Shell Type": shell_type if shell_type else "nan",
            "Administration Dosages (mg/kg)": float(dosage),
            "Time point (h)": float(time_point),
            "Tumor Site": tumor_site if tumor_site else "nan",
        }

        input_df = pd.DataFrame([raw_row])[FEATURES_ORDER]

        # The pipeline handles encoding internally — no extra step needed
        ...
        with ref_col1:
            st.info(
                ...
                "* **High Retention:** bio-distribution efficiency **above or equal to the dataset median**.\n"
                "* **Low Retention:** below the dataset median — consider optimizing PEGylation or size."
            )
        with ref_col2:
            st.success(
                "**Targeting Selectivity Index Classification:**\n\n"
                "* **High Selectivity:** tumor-to-organ ratio **>= median** — minimized off-target effects.\n"
                "* **Low Selectivity:** relatively high off-target accumulation — consider changing Shell Type."
            )
        st.caption("*Note: Thresholds are computed from the median values of the historical Nano-Drug Delivery Silver Dataset.*")
